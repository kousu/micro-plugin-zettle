...
local function openPath(bp, uri)
	...
	local function openWithSystem(uri)
		...
		micro.After(3 * time.Second, function()
			micro.InfoBar():Reset()
		end)
	end
...
function ToggleTodo(bp)
	...
	if not (chkPos and line:match("^%s*-%s+%[[ x]%]")) then
		return false
	end
	-- checkbox occupies 0-based columns (chkPos-1)..(chkPos+1)
	if bp.Cursor.X < chkPos - 1 or bp.Cursor.X > chkPos + 1 then
		return false
	end
...
function NavigateBack(bp)
	...
	local inBounds = prev.y < bp.Buf:LinesNum() and prev.x <= #bp.Buf:Line(prev.y)
	if inBounds then
...
function init()
	...
	if zettleRoot ~= nil then
		...
		micro.After(5 * time.Second, function()
			micro.InfoBar():Reset()
		end)
